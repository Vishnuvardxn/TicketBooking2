...
import SalesReportPrint from '../components/SalesReportPrint'
...
const Search = () => {
	...
	const [showtimesForReport, setShowtimesForReport] = useState(null)
	const [filtersForReport, setFiltersForReport] = useState(null)
	...
	const handlePrintSalesReport = () => {
		const activeFilters = {
			cinema: filterCinema,
			theater: filterTheater,
			movie: filterMovie,
			date: filterDate,
			dateFrom: filterDateFrom,
			dateTo: filterDateTo,
			isPastDate: filterPastDate,
			isToday: filterToday,
			isFutureDate: filterFutureDate,
			time: filterTime,
			timeFrom: filterTimeFrom,
			timeTo: filterTimeTo,
			isReleased: filterReleaseTrue,
			isUnreleased: filterReleaseFalse
		}
		setShowtimesForReport(filteredShowtimes)
		setFiltersForReport(activeFilters)
		setTimeout(() => {
			window.print()
			setShowtimesForReport(null)
			setFiltersForReport(null)
		}, 100)
	}

	const handleDownloadCsv = () => {
		const headers = ['Cinema', 'Theater', 'Movie', 'Date', 'Time', 'Booked Seats', 'Price/Ticket', 'Revenue']
		const csvRows = [headers.join(',')]

		filteredShowtimes.forEach((showtime) => {
			const showtimeDate = new Date(showtime.showtime)
			const date = showtimeDate.toISOString().split('T')[0]
			const time = showtimeDate.toTimeString().split(' ')[0].substring(0, 5)
			const revenue = showtime.seats.length * showtime.price
			const movieTitle = `"${showtime.movie.name.replace(/"/g, '""')}"` // Handle quotes in movie titles
	...
	return (
		<>
			<div className="flex min-h-screen flex-col gap-4 bg-gradient-to-br from-indigo-900 to-blue-500 pb-8 text-gray-900 print:hidden sm:gap-8">
				<Navbar />
				<div className="mx-4 flex h-fit flex-col gap-2 rounded-lg bg-gradient-to-br from-indigo-200 to-blue-100 p-4 drop-shadow-xl sm:mx-8 sm:p-6">
					...
					<div className="flex items-end">
						...
						<div className="ml-auto flex flex-wrap items-center justify-end gap-2 px-1">
							{isFetchingShowtimesDone && (
								<div className="flex items-center gap-1 text-sm font-medium">
									<InformationCircleIcon className="h-5 w-5" /> Showing {filteredShowtimes.length}{' '}
									filtered showtimes
								</div>
							)}
							{auth.role === 'admin' && (
								<button
									className="flex w-fit items-center justify-center gap-1 rounded bg-gradient-to-r from-green-600 to-emerald-500 py-1 pl-2 pr-1.5 text-sm font-medium text-white hover:from-green-500 hover:to-emerald-400 disabled:from-slate-500 disabled:to-slate-400 md:min-w-fit"
							...
							{auth.role === 'admin' && (
								<button
									className="flex w-fit items-center justify-center gap-1 rounded bg-gradient-to-r from-sky-600 to-cyan-500 py-1 pl-2 pr-1.5 text-sm font-medium text-white hover:from-sky-500 hover:to-cyan-400 disabled:from-slate-500 disabled:to-slate-400 md:min-w-fit"
			...
			{showtimesForReport && <SalesReportPrint showtimes={showtimesForReport} filters={filtersForReport} />}
		</>
	)
}
